 ...
 */
@WebServlet(name = "RamFriquencyServiceServlet")
public class RamFriquencyServiceServlet extends HttpServlet {

    private static final String PARAMETR_METHOD = "method";
    private static final String PARAMETR_ID = "id";
    private static final String PARAMETR_FRIQUENCIES = "friquency";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(GET_ALL_METHOD.equalsIgnoreCase(queryMethod)) {
            List<RamFriquency> ramFriquencies = facadeDAO.getRamFriquencyDAO().loadAll();
            try {
                PrintWriter out = response.getWriter();
                Gson gson = new Gson();
                gson.toJson(ramFriquencies, out);
            ...
            RamFriquency friquencies = new RamFriquency(friquenciesString, identityString);
            boolean created = facadeDAO.getRamFriquencyDAO().add(friquencies);
            try{
                PrintWriter out = response.getWriter();
                if(created){
                    response.setStatus(HttpServletResponse.SC_OK);
            ...
            String identityString = request.getParameter(PARAMETR_IDENTITY);
            RamFriquency friquencies = new RamFriquency(friquenciesString, identityString);
            boolean updated = facadeDAO.getRamFriquencyDAO().update(friquencies);
            try {
                PrintWriter out = response.getWriter();
                if (updated) {
                    response.setStatus(HttpServletResponse.SC_OK);
                    ...
                    response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                    out.print("ОШИБКА при ОБНОВЛЕНИИ параметра ЧАСТОТА");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            boolean deleted = facadeDAO.getRamFriquencyDAO().delete(id);
            try {
                PrintWriter out = response.getWriter();
                if (deleted) {
                    response.setStatus(HttpServletResponse.SC_OK);
