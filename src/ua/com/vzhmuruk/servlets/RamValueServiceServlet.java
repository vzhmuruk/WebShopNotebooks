 ...
 */
@WebServlet(name = "RamValueServiceServlet")
public class RamValueServiceServlet extends HttpServlet {

    private static final String PARAMETR_METHOD = "method";
    private static final String PARAMETR_ID = "id";
    private static final String PARAMETR_SIZE = "memory_size";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(GET_ALL_METHOD.equalsIgnoreCase(queryMethod)) {
            List<RamValue> types = facadeDAO.getRamValueDAO().loadAll();
            try {
            ...
            String sizeString = request.getParameter(PARAMETR_SIZE);
            RamValue ramValue = new RamValue(sizeString);
            boolean created = facadeDAO.getRamValueDAO().add(ramValue);
            try{
                PrintWriter out = response.getWriter();
                if(created){
                    response.setStatus(HttpServletResponse.SC_OK);
                    out.print("Параметр ОБЪЕМ ОЗУ создан");
                }else{
                    response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                    out.print("Ошибка при создании параметра ОБЪЕМ ОЗУ");
                }
            }catch(Exception e){
                e.printStackTrace();
            }
            ...
            String sizeString = request.getParameter(PARAMETR_SIZE);
            RamValue ramValue = new RamValue(sizeString);
            boolean updated = facadeDAO.getRamValueDAO().add(ramValue);
            try {
                PrintWriter out = response.getWriter();
                if (updated) {
                    response.setStatus(HttpServletResponse.SC_OK);
                    out.print("Параметр ОБЪЕМ ОЗУ ОБНОВЛЕН успешно!");
                } else {
                    response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                    out.print("ОШИБКА при ОБНОВЛЕНИИ параметра ОБЪЕМ ОЗУ");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            boolean deleted = facadeDAO.getRamValueDAO().delete(id);
            try {
                PrintWriter out = response.getWriter();
                if (deleted) {
                    response.setStatus(HttpServletResponse.SC_OK);
